...
public class DataSyncControl_ResponseDownloadPOIImage extends DataSyncControl_ResponseFileBase{
	...
	public DataSyncControl_ResponseDownloadPOIImage(int iRequestId) {
		super(iRequestId);
	}
...
//					jniPointControl_new.Instance().PrepareGetNextImageInfo(jniPointControl_new.PNT_SYNC_IMAGE_STATES_SUCESS,getM_StrPassParam01());
					...
					// No Image
//					jniPointControl_new.Instance().PrepareGetNextImageInfo(jniPointControl_new.PNT_SYNC_IMAGE_STATES_NO_FILE,getM_StrPassParam01());
				}
			} else {
				//prase error
//				jniPointControl_new.Instance().PrepareGetNextImageInfo(jniPointControl_new.PNT_SYNC_IMAGE_STATES_PARSE_ERROR,getM_StrPassParam01());
	...
	private void saveFile() {
		//TODO
		// path getM_StrPassParam01();
		if (null == strPOIImage.getBytes()) {
			return;
		}
		
		byte[] content = Base64.decode(strPOIImage.getBytes(), Base64.DEFAULT);
		String path = getM_strFilePath();
		if(content == null)
		{
			return ;
		}
