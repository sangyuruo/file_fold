 ...
 * @author sangjun
 */
public class EglWindowSurfaceForMediacoderStrategy extends
		AbstractEglSurfaceStrategy implements EglSurfaceStrategy {

	private EglWindowSurfaceForMediacoderStrategy() {
	}

	static EglWindowSurfaceForMediacoderStrategy strategy = null;

	public static synchronized EglSurfaceStrategy getInstance() {
		if (null == strategy) {
			strategy = new EglWindowSurfaceForMediacoderStrategy();
		}
		return strategy;
	}
...
//        };
	...
	public EGLSurface createSurface(Surface surface) {
		 ...
		 * Create an EGL surface we can render into.
		 */
		EGLSurface eglSurface = mEgl.eglCreateWindowSurface(mEglDisplay,
