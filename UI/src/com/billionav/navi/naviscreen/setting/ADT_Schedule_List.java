...
public class ADT_Schedule_List extends ActivityBase implements OnSearchFinishedListener, OnScreenBackListener{
...
//		handler.sendEmptyMessageDelayed(1, 3000);
	...
	private void initialize() {
		UIC_SreachCommon.setCurListActivityclass(ADT_Schedule_List.class);
//		cdialog = new CustomDialog(ADT_Srch_List.this);
		setDefaultBackground();
		PointTools.getInstance().requestCenterName(
				new PointDetailInfoListener() {

					@Override
					public void onPointDetailInfoRequstFinished(String name,
							String address, String tel, long distance) {
						Log.d("UIMsgControl",
								"onPointDetailInfoRequstFinished : name = "
										+ name);
		...
		listContainer.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (HybridUSTools.getInstance().isTheSamePoint(
						bean.getLonlat()[0],
						bean.getLonlat()[1])) {
					CustomDialog dialog = new CustomDialog(ADT_Schedule_List.this);
				...
				BackSearchWinChange(ADT_Schedule_Activity.class);
			}
		});
	}
...
//			}
	...
	class SearchListAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if(convertView == null) {
					convertView = new ListItemSearchResult(ADT_Schedule_List.this, name, address, dStr, R.drawable.navicloud_and_446a);
				...
				if(convertView != null){
					((ListItemSearchResult)convertView).setRouteListener(new OnClickListener() {
						...
						public void onClick(View v) {
							...
							getBundleNavi().put("SearchPointBean", (SearchPointBean) v.getTag());
