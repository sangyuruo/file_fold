...
public class UserControl_ResponseGetResource  extends UserControl_ResponseBase {
	...
	public UserControl_ResponseGetResource(int iRequestId) {
		super(iRequestId);
	...
	public void doResponse() {
		...
		cTrigger.m_iTriggerID = NSTriggerID.UIC_MN_TRG_UC_GET_RESOURCE;
	...
	public boolean ParseResultInfo(byte[] inputData) {
		if(0 != "text/xml".compareTo(getRecvContentType()))
		{
			// if the data from server is not an xml string,
			//and then it says this data is byte data,
			//this data is user photo
			UserControl_ManagerIF .Instance().SetPhotoByteToUserInfo(inputData);
