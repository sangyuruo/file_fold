...
public class VersionControl_ResponseGetTopDataVersion  extends VersionControl_ResponseBase{
	private int iStatus = UC_RESPONES_SUC;
	private int iDetails = 0;
	
	public static final String TAG = "map";
	private static String TOP_VERSION_KEY = "top_data_version";
	private static String MIN_VERSION_KEY = "min_data_version";
	
	public VersionControl_ResponseGetTopDataVersion(int iRequestId) {
		super(iRequestId);
	}
	...
	public void doResponse() {
		...
		if (PResponse.RES_CODE_NO_ERROR == iResCode) {
			try{
				...
				VersionControl_ManagerIF.Instance().setM_strMinDataVersion(strMinVersion);
				VersionControl_ManagerIF.Instance().setM_strTopDataVersion(strTopVersion);
				Log.d("[VersionControl]", "[VersionControl]: GetTopDataVersion strTopVersion : " + strTopVersion + "  strMinVersion: " + strMinVersion);
			}catch(Exception ex) {
				iStatus =  UC_RESPONES_LOC_FAIL;
				iDetails = UC_DETAILS_JSON_PRASE_ERROR;
				Log.d("[VersionControl]", "[VersionControl]: GetTopDataVersion Response Exception");
			}
		} else {
		...
		Log.d("[VersionControl]", "[VersionControl]: GetTopDataVersion Response iStatus = " + iStatus + " iDetails=" +iDetails);
